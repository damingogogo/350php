...
Assert-Contains 'resources\views\platform\home.blade.php' 'hero-feature-board' 'guest landing fills the left hero overview area with platform content'
Assert-Contains 'resources\views\platform\home.blade.php' '教师发布' 'guest landing explains the resource sharing workflow'
...
Assert-Contains 'resources\views\platform\dashboard.blade.php' 'page-hero' 'dashboard uses a focused compact hero instead of an oversized title block'
Assert-Contains 'resources\views\platform\dashboard.blade.php' 'focus-strip' 'dashboard highlights key platform work areas'
...
Assert-Contains 'resources\views\platform\layout.blade.php' "platform.profile[\s\S]*platform.backend" 'top navigation places personal center before role backend'
...
Assert-Contains 'resources\views\platform\backend-admin.blade.php' "backendSection === 'questions'" 'admin question bank management has its own sidebar section'
Assert-Contains 'resources\views\platform\backend-admin.blade.php' 'admin-collapsible' 'admin edit/delete records can collapse and expand'
...
Assert-Contains 'resources\views\platform\backend-teacher.blade.php' 'teacherFavorites' 'teacher backend shows favorite learning resources'
Assert-Contains 'resources\views\platform\backend-teacher.blade.php' 'teacherDownloads' 'teacher backend shows downloaded learning resources'
Assert-Contains 'resources\views\platform\backend-teacher.blade.php' "section' => 'history'" 'teacher backend more links open a fuller learning record page'
...
Assert-Contains 'resources\views\platform\backend-student.blade.php' 'name="teacher_id"' 'student homework submission can choose a target teacher'
Assert-Contains 'resources\views\platform\backend-student.blade.php' 'student-board-helper' 'student resource pool page fills the right-side guidance area'
...
Assert-Contains 'resources\views\platform\resource-show.blade.php' 'format-options' 'resource detail file formats are clickable choices'
...
Assert-Contains 'resources\views\platform\resources.blade.php' 'resource-card' 'resource list cards visually emphasize each module'
Assert-Contains 'resources\views\platform\questions.blade.php' 'question-card' 'question list cards visually emphasize each module'
...
Assert-Contains 'app\Http\Controllers\PlatformController.php' 'teacherFavorites' 'controller provides teacher favorite learning records'
Assert-Contains 'app\Http\Controllers\PlatformController.php' 'teacherDownloads' 'controller provides teacher download learning records'
Assert-Contains 'app\Http\Controllers\PlatformController.php' "'teacher_id' => 'required\|exists:users,id'" 'homework submission validates the selected teacher'
...
Assert-Contains 'app\Models\HomeworkSubmission.php' 'teacher_id' 'homework submissions store the target teacher'
Assert-Contains 'app\Models\HomeworkSubmission.php' 'function teacher' 'homework submissions relate to the chosen teacher'
...
Assert-Contains 'database\migrations\2024_01_01_000011_add_teacher_id_to_homework_submissions_table.php' 'teacher_id' 'homework submissions migration adds target teacher'
...
Assert-Contains 'database\seeders\DatabaseSeeder.php' 'teacherLearningSeedPairs' 'seed data includes teacher favorites and downloads for backend display'
